...
struct data
{
    ...
    //LSByte first
    const uint16_t      distance;         //13 : 0
    const bool          strength_warn;    //14
    const bool          invalid_data;     //15
...
struct packet
{
    ...
    const uint8_t    index;//Packet #0 - #89, 4 each {0xA0 - 0xF9}
    double      speed;// 64th of RPM
